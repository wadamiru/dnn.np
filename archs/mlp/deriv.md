...
$$
Y = \mathrm{GELU}_{\tanh}(X) = \frac{1}{2} X \odot \left( \mathbf{1}_{N \times d} + \tanh(U) \right)
$$

where:

$$
U = \sqrt{\frac{2}{\pi}} \left( X + 0.044715 \, X \odot X \odot X \right)
$$

Applying the Hadamard product rule to $Y$:

$$
\mathrm{d}Y = \frac{1}{2} (\mathrm{d}X) \odot \left( \mathbf{1}_{N \times d} + \tanh(U) \right) + \frac{1}{2} X \odot \mathrm{d}\tanh(U)
$$
